...
@interface CZWTextView () <UITextViewDelegate>
...
- (void)baseSetUp{
    ...
    //self.inputAccessoryView = [[EditTextInputAccessoryView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 44) target:self doneAction:@selector(done:) clearAction:@selector(clear:)];
...
{
    if ([self.tableView.delegate respondsToSelector:@selector(tableView:textViewDidBeginEditing:indexPath:)]) {
        NSIndexPath *indexPath = [self.tableView indexPathForCell:self.cell];
        [(id<CZWCellHeightUpdateWithTextProtocol>)self.tableView.delegate tableView:self.tableView textViewDidBeginEditing:textView indexPath:indexPath];
    }
}


-(void)textViewDidEndEditing:(UITextView *)textView{
    if ([self.tableView.delegate respondsToSelector:@selector(tableView:textViewDidEndEditing:indexPath:)]) {
        NSIndexPath *indexPath = [self.tableView indexPathForCell:self.cell];
        [(id<CZWCellHeightUpdateWithTextProtocol>)self.tableView.delegate tableView:self.tableView textViewDidEndEditing:textView indexPath:indexPath];
    }
}

- (void)textViewDidChangeSelection:(UITextView *)textView {
    if ([self.tableView.delegate respondsToSelector:@selector(tableView:textViewDidChangeSelection:)]) {
        [(id<CZWCellHeightUpdateWithTextProtocol>)self.tableView.delegate tableView:self.tableView textViewDidChangeSelection:textView];
    }
}

- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([self.tableView.delegate respondsToSelector:@selector(tableView:textView:shouldChangeTextInRange:replacementText:)]) {
        if ([[self.tableView.delegate class] conformsToProtocol:@protocol(CZWCellHeightUpdateWithTextProtocol)]) {
            id<CZWCellHeightUpdateWithTextProtocol> delegate = (id<CZWCellHeightUpdateWithTextProtocol>)self.tableView.delegate;
            return [delegate tableView:self.tableView textView:textView shouldChangeTextInRange:range replacementText:text];
        }
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    [self.tableView scrollToRowAtIndexPath:[self.tableView indexPathForCell:self.cell] atScrollPosition:UITableViewScrollPositionTop animated:YES];
...
{
    if ([self.tableView.delegate conformsToProtocol:@protocol(CZWCellHeightUpdateWithTextProtocol)]) {
       ...
       // _text = self.textView.text;
        ...
        [delegate tableView:self.tableView updatedText:theTextView.text atIndexPath:indexPath];
        
        CGFloat newHeight = [self newViewHeight];
        CGFloat oldHeight = theTextView.frame.size.height;
        ...
        if (fabs(newHeight - oldHeight) > 0.01) {
            ...
            // update the height
            if ([delegate respondsToSelector:@selector(tableView: updatedText: atIndexPath:)]) {
